...
def main():
    ...
    for seq_id in sequence_ids:
        ed_avges[seq_id] = {}
        for  subdir in edit_distances[seq_id].keys():
            na_num[subdir] = 0
    ...
    bar_seq_1 = []
    for subdir in edit_distances[sequence_ids[0]].keys():
        bar_seq_1.append(ed_avges[sequence_ids[0]][subdir])
    
    
    bar_seq_2 = []
    for subdir in edit_distances[sequence_ids[1]].keys():
            bar_seq_2.append(ed_avges[sequence_ids[1]][subdir])

    bar_3_nas = list(na_num.values())
    

    xticks = list(edit_distances[sequence_ids[0]].keys())
    xticks = [x[3:].split("_") for x in xticks]
    xticks = [x[0] + "%" + " and " + x[1] + "%" for x in xticks]
    ...
    ax.set_xlabel('Relative abundance for the HIV-1 sequences', fontweight = "bold")
    ...
    plt.savefig(args.output_dir + '/edit_distance_to_consensus.pdf')
