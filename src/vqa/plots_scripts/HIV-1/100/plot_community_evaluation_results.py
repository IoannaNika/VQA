...
def main():
    ...
    for sub_directory in os.listdir(args.input_dir):
        ...
        if os.path.exists(input_file):
            ...
            for region in regions: 
                ...
                results[sub_directory][region]['n_pred_communities'] = n_pred_communities[regions == region][0]
    ...
    bars_homogeneity = [avg_results[subdir]['homogeneity'] for subdir in avg_results.keys()]
    ...
    bars_completeness = [avg_results[subdir]['completeness'] for subdir in avg_results.keys()]
    ...
    bars_ratio_communities = [avg_results[subdir]['ratio_communities'] for subdir in avg_results.keys()]
    ...
    plt.bar(np.arange(len(avg_results.keys())) + 0.4, bars_ratio_communities, color='#DDCC77', width=0.2, label='Ratio of predicted communities\nto true communities')
    ...
    for subdir in avg_results.keys():
        ...
        if subdir[1][0] == '0':
            subdir[1] ="0." + subdir[1][1:]
        if subdir[2][0] == '0':
            subdir[2] ="0." + subdir[2][1:]
        subdir = subdir[1] + "% " + "and " + subdir[2] + "%"
        xticks.append(subdir)

        
    plt.xticks(np.arange(len(avg_results.keys())) + 0.3, xticks, fontsize=15)
    # put legend outside of the plot in the middle of the right side
    plt.legend(fontsize=15)
    plt.ylabel('Average scores over genomic regions', fontsize=20, fontweight ="bold")
    plt.xlabel('Relative abundances for the two simulated HIV-1 sequences', fontsize=20, fontweight ="bold")
    # plt.title('HIV-1: Community evaluation results (100x), testing for relative abundance', fontsize=20, fontweight='bold')
    plt.tight_layout()
    plt.savefig(os.path.join(args.output_dir, "community_evaluation_results.pdf"))
    plt.close(fig)
