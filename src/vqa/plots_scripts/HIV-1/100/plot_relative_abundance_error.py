...
def main():
    ...
    for subdirs in true_abundances[seq_ids[0]].keys():
    ...
    bar_2 = []
    var_2 = []
    for  subdirs in true_abundances[seq_ids[1]].keys():
   ...
   # plot the relative abundance error average per genomic region and per sequence and plot error bars 
    ...
    xticks = []
    for subdir in true_abundances[seq_ids[0]].keys():
  ...
  # x-ticks are the subdirectories
    ...
    plt.xlabel("Abundance distribution", fontweight='bold')
    plt.ylabel("Absolute average relative abundance error", fontweight='bold')
    plt.legend()
    plt.tight_layout()

    plt.savefig(args.output_dir + '/' + 'relative_abundance_error.pdf')
    plt.close()
